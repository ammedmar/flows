...
We will rarely require the more stringent definition of a smooth map between manifolds with corners, as the only time we will consider fiber products over manifolds with (non-empty) corners will be when those products are zero-dimensional, for which we give an ad hoc treatment.
In those other few cases where a map has a codomain with non-empty boundary or corners, such as the pullback maps from $W \times_M V$ to $V$ or $W$, they will also satisfy Joyce's stronger conditions.
...
\begin{definition}\label{V: maps are co-oriented}
	...
	Let $\cman(M)$ denote the set of proper co-oriented c-manifolds over $M$.

	For an element of $\cman(M)$, we write $|W|$ for the \textbf{codimension} $|W|=\dim(M)-\dim(W)$.
	Let $\cman^*(M)$ be the free abelian group generated by $\cman(M)$, graded by the \textbf{codimension} $|W|$, modulo the following relations:
	\begin{enumerate}
		\item ${V \sqcup W} = V + W$,
...
By these relations any element of $\cman^*(M)$ is represented by a single map from a likely disconnected manifold with corners, as in particular one can find as many ``copies'' as one needs of any manifold with corners embedded in $\R^\infty$.
...
Details can be found in \cite{Lipy14} and \cite[Lemma 3.26]{medina2022foundations}.
...
The following is Theorem 6.5 of \cite{medina2022foundations}:
